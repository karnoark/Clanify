...
export const useAuthStore = create<AuthState & AuthActions>((set, get) => {
  ...
  storeManager.registerStore(
    {
      name: 'auth',
      critical: true, // Auth is critical for app function
  ...
  return {
    ...
    initialize: async (): Promise<void> => {
      try {
        ...
        if (session) {
          ...
          // console.log(
          //   'auth/initialize:-> saved session & user info in zustand ',
          // );
          saveToStorage('session', session);
          saveToStorage('user', user);
          // Emit auth completed event
          await eventManager.emit(StoreEvent.AUTH_COMPLETED, {
            userId: user.id,
          });
        ...
        const handledError = AuthErrorHandler.handleError(error as Error);
        set({ error: handledError, initialized: true });

        await eventManager.emit(StoreEvent.AUTH_ERROR, {
          error: handledError,
        });
      } finally {
        set({ isLoading: false });
    ...
    signIn: async (credentials: SignInCredentials) => {
      ...
      try {
        // Clear existing state before signin
        set({
          user: null,
          session: null,
          isLoading: true,
    ...
    refreshSession: async () => {
      ...
      if (error) {
        await eventManager.emit(StoreEvent.AUTH_ERROR, {
          error: AuthErrorHandler.handleError(error),
        });
        throw error;
      }
      ...
      if (data.session) {
        const user: User = {
          ...
          role: data.session.user.user_metadata?.role ?? 'regular',
        };

        set({ session: data.session, user });
        saveToStorage('session', data.session);
        saveToStorage('user', user);
...
// Add new hooks for common auth checks
